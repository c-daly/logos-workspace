...
5. **Windowed encoding for longer video**: Uniform subsampling to 64 frames is temporally incoherent for videos longer than a natural clip — V-JEPA was trained on coherent segments, not scattered frames. A windowed approach (encode each 64-frame window, then mean-pool across windows) preserves meaningful local temporal structure. Per-window embeddings (not mean-pooled) would additionally enable temporal retrieval — finding the specific moment rather than just the video. Stride choice (64 = no overlap, 32 = 50% overlap) and whether to pool or store per-window are open. Not determined yet — note for when video ingestion and the encoder pipeline are designed.

4. **Universal HCG search**: If the explorer proves useful, search should eventually span all three CWM layers — not just grounded (CWM-G) nodes but also abstract (CWM-A) and emotional (CWM-E) nodes. The motivating use case is evaluation: enter a text query and see which nodes across all layers match, to understand how well the model's embedding space aligns with Sophia's full knowledge structure. This requires either (a) a common embedding space across all layers (all nodes gain grounded embeddings in addition to text embeddings, searchable via `grounded_embeddings`), or (b) separate ranked lists per layer merged in the UI. The `Result` type must carry `cwm_layer: "g" | "a" | "e"`, `node_type`, `neo4j_id`, and enough `properties` to make results interpretable. Avoid hard-coding MSR-VTT-specific result shapes in Phase 3 to leave this path open.
