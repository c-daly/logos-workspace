...
4. **Universal HCG search**: If the explorer proves useful, search may be extended to cover the full `grounded_embeddings` collection across all HCG nodes, not just MSR-VTT. The `Result` type should carry enough HCG context (`node_type`, `neo4j_id`, `properties`) to remain useful beyond video retrieval. Consider this when building the Phase 3 frontend — avoid hard-coding MSR-VTT-specific result shapes.
