...
FINDINGS FROM 75 EXPERIMENTS (use this to inform next round):
The dominant architecture is residual(1024,6)→transformer(768,6) with num_tokens=24. Best val txtR@1=0.377.
Top configs all share: high text infonce weight (0.85-0.93), low temp (0.021-0.025), batch_size=128-256, long schedules (600-900 epochs).
Token-level (num_tokens=24/32) has been thoroughly explored and matches or exceeds pooled approaches.
Image retrieval is saturated (imgR@5 ~0.92-0.93) — focus entirely on text retrieval.

The search is PLATEAUING around 0.37. To break through 0.42 requires genuinely new ideas, NOT variations on the residual→transformer family.
Ideas that have NOT been tried and may help:
- Cross-attention: attend JEPA tokens to CLIP text tokens directly (requires passing text embeddings at inference — likely out of scope)
- Asymmetric batch: use more text captions per video to increase InfoNCE negative density
- Token selection: learn which of the 32 tokens are most informative (attention pooling or gating)
- Ensemble head: train multiple projectors and average at inference
- Different temporal aggregation: weighted mean pool (learnable weights over 32 tokens) instead of transformer
- Much higher infonce temperature search: try temp=0.05-0.10 (current best is ~0.02, maybe too sharp)
- Two-stage training: first train on image retrieval to convergence, then fine-tune on text only
- Curriculum: start with num_tokens=8, gradually increase to 32 during training
Keep batch_size ≤ 256. For token experiments, prefer batch_size=128.
